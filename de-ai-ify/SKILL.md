---
name: de-ai-ify
description: Edit or audit drafted text to remove AI-slop patterns while preserving the writer's voice. Use whenever the user asks to de-AI-ify, de-slop, humanize, "make it not sound like AI", audit a draft for AI patterns, or tighten writing — and ALWAYS run this pass on drafted content before it goes into a styled report, publication, or the regainflow-report skill. Trigger even if the user only says "clean this up" about prose destined for publishing.
---
...
- **Short drafts (under ~300 words):** return the edited draft inline in chat, followed by What changed.
- **Longer drafts:** write the edited draft to a markdown file (e.g. `/mnt/user-data/outputs/<slug>-edited.md`) and present it so the user gets a rendered markdown preview. Put the What changed section in chat, not in the file.
- **Detect requests:** findings report in chat only. No file.

## What to ask for

- If the user has not provided a draft, ask them to paste or upload it.
...
4. For an edit, make the minimum effective changes, then run the self-check below against the edited draft.
5. If any check fails, fix the draft and check again.
6. Output the full edited draft (inline or as a presented .md file per Output format) and a short **What changed** section.

## Self-check before returning an edit

- No banned words remain.
- No pattern from Patterns to cut remains, unless it is demonstrably the writer's own voice (and you can point to it elsewhere in the original).
- The draft's meaning is unchanged: no invented facts, stats, sources, or opinions.
- Distinctive human sentences from the original survive untouched.
- Every remaining sentence passes the portability test or earns its place with voice.
- The ending lands on a concrete point, takeaway, or next action, not a recap or aphorism.

## Handoff

If the cleaned content is destined for a styled PDF report (or the user mentions the report/publication pipeline), offer to continue with the **regainflow-report** skill: it takes the de-AI-ified markdown and produces the branded PDF with cover, contents, figures, sources, and authors pages.
